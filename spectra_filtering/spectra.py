...
def raw_periodogram(
    ...
    """
    length_of_dataset = int(len(x))
    if detrend:
        mean = x.mean()
        x = x - mean  # remove the mean
        trend = linregress(range(length_of_dataset), x)
        x = x - trend.intercept - trend.slope * np.arange(length_of_dataset)  # detrended data

    # Apply window
    win = signal.get_window(window, length_of_dataset)
    x = x * win

    # Compute FFT and squared magnitude
    fft = np.fft.rfft(x)
    psd = np.abs(fft) ** 2

    # Frequency axis

    window_energy = np.sum(win**2)
    freq = np.fft.rfftfreq(length_of_dataset, dt_days)
    df = 1 / (length_of_dataset * dt_days)
    psd = psd / (window_energy * length_of_dataset * df)  # Normalize by window energy and N

    psd = psd*2  # One-sided scaling
    psd[0] = psd[0]* 0.5 # DC component
    if length_of_dataset % 2 == 0:
        psd[-1] = psd[-1]*0.5  # Nyquist component
